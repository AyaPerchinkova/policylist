...
var _ = Describe("Config", func() {
	...
	Context("LoadFirestoreConfig", func() {
		...
		When("firestore collection is set", func() {
			It("should return correctly set config and no error", func() {
				//config, err := config.LoadFirestoreConfig()
				//	Expect(err).ToNot(HaveOccurred())
				//	Expect(config.Collection).To(Equal(firestoreCollection))
			})
		})
	})
})
